...
async function upsertMetadataForIpEntry(ipEntryId, body) {
    ...
    const data = {
        ...
        mbManufacturer: emptyToNull(Motherboard.Manufacturer ?? Motherboard.manufacturer),
...
export async function patchMetadataByIp(ip, patchBody) {
    ...
    for (const k of ["OS", "System", "Motherboard", "BIOS", "CPU"]) {
        if (patchBody?.[k]) merged[k] = { ...(existing?.[k] || {}), ...(patchBody[k] || {}) };
    }
...
export async function statsService(includeMeta = false) {
    ...
    const avgRam = ramTotals.length ? ramTotals.reduce((a, b) => a + b, 0) / ramTotals.length : 0;
    ...
    const nicTop = nicTopRaw.map((r) => ({ key: String(r.speedNorm), count: r.count }));
    ...
    return {
        stats: {
            ...
            coveragePct: totalIpEntries ? Math.round((totalWithMeta / totalIpEntries) * 100) : 100,
        },
