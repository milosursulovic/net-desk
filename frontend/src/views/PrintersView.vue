<template>
  <div class="space-y-4">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      <h1 class="text-2xl font-bold text-ink" style="font-family: var(--font-display)">{{ t('nav.printers') }}</h1>
      <div class="flex flex-wrap items-center gap-2">
        <AppButton variant="success" @click="openCreate">{{ t('printers.add') }}</AppButton>
        <AppButton variant="secondary" @click="exportXlsx">{{ t('home.exportXlsx') }}</AppButton>
      </div>
    </div>
    ...
    <div class="space-y-3">
      ...
      <div class="relative">
        <input v-model="searchInput" @input="onSearchInput" type="text"
          ...
          :aria-label="t('printers.searchAriaLabel')" />
      ...
      <p class="text-sm text-ink-muted">{{ t('printers.shown', { shown: items.length, total }) }}</p>
    </div>
    ...
    <SlideOverPanel :open="showModal" :title="editId ? t('printers.editTitle') : t('printers.newTitle')" @close="closeModal">
      <div class="space-y-4">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <FormInput v-model.trim="form.name" :label="t('printers.colName')" placeholder="HP LaserJet 400" />
          <FormInput v-model.trim="form.manufacturer" :label="t('printers.manufacturer')" placeholder="HP" />
          <FormInput v-model.trim="form.model" label="Model" placeholder="M401dne" />
          <FormInput v-model.trim="form.serial" :label="t('printers.serial')" />
          <div>
            <label class="text-sm text-ink-secondary">{{ t('common.department') }}</label>
            <GroupSelect
          ...
          <div>
            <label class="text-sm text-ink-secondary">{{ t('addIp.siteLabel') }}</label>
            <select v-model="form.site" class="app-input w-full">
              <option v-for="o in SITE_OPTIONS" :key="o.value" :value="o.value">{{ o.label }}</option>
            </select>
          </div>
          <div>
            <label class="text-sm text-ink-secondary">{{ t('printers.connectionTypeLabel') }}</label>
          ...
          <div class="flex items-center gap-2 mt-6">
            ...
            <label for="shared" class="text-sm text-ink">{{ t('printers.shared') }}</label>
          </div>
        </div>

        <div class="flex gap-2 justify-end">
          <AppButton variant="neutral" @click="closeModal">{{ t('common.cancel') }}</AppButton>
    ...
    <SlideOverPanel :open="toolsOpen" width-class="sm:w-[720px]" @close="closeTools">
      <template #title>
        {{ t('printers.connectingTitle', { name: toolsPrinter?.name || '—' }) }}
      ...
      <div class="space-y-4">
        <div class="grid grid-cols-1 gap-3">
          <div class="border border-line rounded-lg p-3 bg-surface-sunken">
            <div class="font-medium text-ink mb-2">{{ t('printers.connectComputer') }}</div>
            <div class="text-xs text-ink-muted mb-1">{{ t('printers.connectComputerHint') }}</div>
            <div class="flex gap-2">
              ...
              <AppButton variant="success" @click="connectComputerFromTools">{{ t('printers.connect') }}</AppButton>
            </div>
          </div>

          <div class="border border-line rounded-lg p-3 bg-surface-sunken">
            <div class="font-medium text-ink mb-2">{{ t('printers.setHost') }}</div>
            <div class="text-xs text-ink-muted mb-1">{{ t('printers.setHostHint') }}</div>
            <div class="flex gap-2">
              <input v-model.trim="toolsForm.hostInput" :placeholder="t('printers.ipOrIdPlaceholder')"
                class="app-input w-full text-sm" @keyup.enter="setHostFromTools" />
              <AppButton variant="primary" @click="setHostFromTools">{{ t('printers.setHostAction') }}</AppButton>
              <AppButton variant="neutral" @click="unsetHostFromTools">{{ t('printers.unsetHostAction') }}</AppButton>
            </div>
          </div>

          <div class="border border-line rounded-lg p-3 bg-surface-sunken">
            <div class="font-medium text-ink mb-2">{{ t('printers.disconnectComputer') }}</div>
            <div class="text-xs text-ink-muted mb-1">{{ t('printers.disconnectComputerHint') }}</div>
            <div class="flex gap-2">
              <input v-model.trim="toolsForm.disconnectInput" :placeholder="t('printers.ipOrIdPlaceholder')"
                class="app-input w-full text-sm" @keyup.enter="disconnectComputerFromTools" />
              <AppButton variant="danger" @click="disconnectComputerFromTools">{{ t('printers.disconnect') }}</AppButton>
            </div>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            <div class="font-medium text-ink mb-1">{{ t('printers.colHost') }}</div>
            <div v-if="toolsLoadingDetails" class="text-sm text-ink-muted">{{ t('common.loading') }}</div>
          ...
          <div>
            <div class="font-medium text-ink mb-1">{{ t('printers.connectedComputers') }}</div>
            <div v-if="toolsLoadingDetails" class="text-sm text-ink-muted">
              {{ t('printers.loadingDetails') }}
            </div>
            <template v-else>
              ...
              <div v-else class="text-sm text-ink-muted">{{ t('printers.noConnectedComputers') }}</div>
            </template>
          </div>
        </div>

        <div class="text-xs text-ink-muted flex flex-wrap gap-x-4 font-mono">
          <span>{{ t('printers.updatedAt', { date: fmtDate(toolsPrinter?.updatedAt) }) }}</span>
          <span>{{ t('printers.createdAt', { date: fmtDate(toolsPrinter?.createdAt) }) }}</span>
        </div>
      </div>
    </SlideOverPanel>
...
async function confirmDelete(p) {
  const ok = await askConfirm(t('printers.confirmDeleteMessage', { name: p.name || t('printers.defaultName') }), { title: t('printers.confirmDeleteTitle') })
  if (!ok) return
  ...
  try {
    ...
    if (!res.ok) throw new Error('HTTP ' + res.status)
    await fetchData()
    showToast(t('printers.deleted'))
  } catch (e) {
    console.error(e)
    showToast(t('printers.errorDelete'), { kind: 'error', duration: 3000 })
  }
}
...
async function connectComputerFromTools() {
  ...
  try {
    ...
    await refreshToolsState(p, t('printers.toastConnected'))
  } catch (e) {
    console.error(e)
  }
}
...
async function disconnectComputerFromTools() {
  ...
  try {
    ...
    await refreshToolsState(p, t('printers.toastDisconnected'))
  } catch (e) {
    console.error(e)
  }
}
...
async function setHostFromTools() {
  ...
  try {
    ...
    await refreshToolsState(p, t('printers.toastHostSet'))
  } catch (e) {
    console.error(e)
  }
}
...
async function unsetHostFromTools() {
  ...
  try {
    ...
    await refreshToolsState(p, t('printers.toastHostUnset'))
  } catch (e) {
    console.error(e)
  }
}
...
async function copy(text) {
  await copyToClipboard(text, t('printers.ipCopied'))
}
...
async function exportXlsx() {
  try {
    ...
    showToast(t('printers.exportReady'))
  } catch (e) {
    console.error('Export greška', e)
    showToast(t('printers.exportError'))
  }
}
